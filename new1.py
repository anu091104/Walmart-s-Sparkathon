...
class InventoryAnalyzer:
    ...
    def process_product(self, product_df, pid):
        ...
        try:
            ...
            # Get category and category-specific costs
            if 'Category' in product_df.columns:
                category = product_df.iloc[-1]['Category']
                product_data['Category'] = category
                params = CATEGORY_PARAMS.get(category, CATEGORY_PARAMS['Unknown'])
            else:
                product_data['Category'] = 'Unknown'
                params = CATEGORY_PARAMS['Unknown']
    ...
    if uploaded_file is not None:
        try:
            ...
            selected_products = st.sidebar.multiselect(
                ...
                default=df['Product ID'].unique()[:3] if len(df['Product ID'].unique()) > 3 else df['Product ID'].unique()
            )
                ...
                if results:
                    ...
                    for result in results:
                        summary_data.append({
                            ...
                            'Ordering Cost': result.get('Ordering Cost', 'N/A'),
                            'Holding Cost': result.get('Holding Cost', 'N/A'),
                            'Lead Time Days': result.get('Lead Time Days', 'N/A')
                        })
                    ...
                    for result in results:
                        ...
                        with col2:
                            st.metric("Ordering Cost", f"${result.get('Ordering Cost', 'N/A')}")
                        with col3:
                            st.metric("Holding Cost", f"${result.get('Holding Cost', 'N/A')}/unit")
                        with col4:
                            st.metric("Lead Time", f"{result.get('Lead Time Days', 'N/A')} days")
                        ...
                        if 'Suggested Price' in result and result['Suggested Price'] != 'N/A':
                            ...
                            with col3:
                                st.metric("Predicted Sales", f"{result['Predicted Sales at Best Price']:.1f} units")
        ...
        # Sample data format - Updated to match exactly what's in the image
        ...
        # Display the sample data in a dataframe with the exact column order
        st.dataframe(
            pd.DataFrame(sample_data)[['Date', 'Product ID', 'Inventory Level', 'Units Sold', 'Price', 'Category']])

        st.caption(
            "Note: Price and Category columns are optional but recommended for pricing and category-specific analysis")
