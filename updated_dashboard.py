...
CATEGORY_PARAMS = {
    ...
    'Unknown': {'ORDERING_COST': 50, 'HOLDING_COST': 2.0, 'LEAD_TIME_DAYS': 7}
}
...
class InventoryAnalyzer:
    ...
    def get_category_params(self, product_df):
        """Get category-specific parameters for a product"""
        if 'Category' in product_df.columns:
            category = product_df.iloc[-1]['Category']
            return CATEGORY_PARAMS.get(category, CATEGORY_PARAMS['Unknown'])
        return CATEGORY_PARAMS['Unknown']
    ...
    def process_product(self, product_df, pid):
        ...
        try:
            ...
            if latest_inventory < reorder_point:
                ...
                decision = f"ORDER {order_qty} units (Cost: ${order_qty * avg_daily_demand * HOLDING_COST + ORDERING_COST:.2f})"
            elif latest_inventory > 1.5 * reorder_point:
                reduce_qty = latest_inventory - reorder_point
                decision = f"REDUCE by {reduce_qty} units (Savings: ${reduce_qty * HOLDING_COST:.2f})"
            else:
    ...
    def analyze_pricing(self, product_df, pid):
        ...
        current_revenue = current_price * price_model.predict([[current_price]])[0]
        suggested_revenue = best_price * best_sales
        ...
        return {
            ...
            'Suggested Revenue': round(suggested_revenue, 2),
...
def create_pricing_analysis_plot(product_data):
    ...
    price_range = np.linspace(
        max(0, pricing_df['Price'].min() * 0.8),
...
def main():
    ...
    if uploaded_file is not None:
                ...
                if results:
                    ...
                    for result in results:
                        summary_data.append({
                            ...
                            'Revenue Impact': result.get('Revenue Impact', 'N/A')
                        })
                    ...
                    for result in results:
                        ...
                        with col3:
                            ...
                            st.metric("Avg Daily Demand", f"{result['Avg Daily Demand']:.1f}")
                        with col4:
                            st.metric("Lead Time (days)", result['Lead Time Days'])
                        ...
                        if 'Suggested Price' in result and result['Suggested Price'] != 'N/A':
                            ...
                            with col3:
                                st.metric("Revenue Impact", f"${result['Revenue Impact']:.2f}")
        ...
        st.caption("Note: Price and Category columns are optional but recommended for better analysis")
